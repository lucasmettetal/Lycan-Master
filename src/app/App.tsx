  ...
  return (
    <div className="relative min-h-full pb-6" style={{ background: "var(--bg-deep)" }}>
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        ...
        {/* Indicateur Pyromane — joueurs huilés */}
        {game.players.some((p) => p.role === "pyromane" && p.status !== "dead") && (game.oiled ?? []).length > 0 && (
          <div className="mx-5 mt-3 p-3 rounded-xl flex items-center gap-3" style={{ background: "rgba(11,10,15,0.6)", border: "1px solid rgba(248,113,113,0.2)" }}>
            <span style={{ fontSize: 16 }}>🔥</span>
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-mono uppercase tracking-wider" style={{ color: "rgba(248,113,113,0.8)" }}>
                Huilés ({(game.oiled ?? []).length})
              </p>
              <p className="text-xs truncate" style={{ fontFamily: "var(--font-body)", color: "var(--text-secondary)" }}>
                {(game.oiled ?? []).map((id) => game.players.find((p) => p.id === id)?.name ?? "?").join(", ")}
              </p>
            </div>
          </div>
        )}

        {/* Indicateur Joueur de Flûte */}
        {game.players.some((p) => p.role === "joueur_flute" && p.status !== "dead") && (game.enchanted ?? []).length > 0 && (
          <div className="mx-5 mt-3 p-3 rounded-xl flex items-center gap-3" style={{ background: "rgba(11,10,15,0.6)", border: "1px solid rgba(168,85,247,0.2)" }}>
            <span style={{ fontSize: 16 }}>🎵</span>
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-mono uppercase tracking-wider" style={{ color: "rgba(192,132,252,0.8)" }}>
                Ensorcelés ({(game.enchanted ?? []).length}/{game.players.filter((p) => p.status !== "dead" && p.role !== "joueur_flute").length})
              </p>
              <p className="text-xs truncate" style={{ fontFamily: "var(--font-body)", color: "var(--text-secondary)" }}>
                {(game.enchanted ?? []).map((id) => game.players.find((p) => p.id === id)?.name ?? "?").join(", ")}
              </p>
            </div>
          </div>
        )}
        ...
        {phase === "end" && game.winner && (() => {
          const w = game.winner;
          const bg = w === "wolves" ? "rgba(139,28,28,0.35)" : w === "angel" ? "rgba(168,85,247,0.2)" : w === "whitewolf" ? "rgba(99,102,241,0.2)" : w === "flute" ? "rgba(168,85,247,0.2)" : w === "pyromane" ? "rgba(234,88,12,0.2)" : "rgba(16,185,129,0.15)";
          const border = w === "wolves" ? "rgba(139,28,28,0.6)" : w === "angel" ? "rgba(168,85,247,0.5)" : w === "whitewolf" ? "rgba(99,102,241,0.5)" : w === "flute" ? "rgba(192,132,252,0.5)" : w === "pyromane" ? "rgba(251,146,60,0.5)" : "rgba(16,185,129,0.4)";
          const color = w === "wolves" ? "#f87171" : w === "angel" ? "#c084fc" : w === "whitewolf" ? "#818cf8" : w === "flute" ? "#c084fc" : w === "pyromane" ? "#fb923c" : "#34d399";
          const emoji = w === "wolves" ? "🐺" : w === "angel" ? "😇" : w === "whitewolf" ? "🐺" : w === "flute" ? "🎵" : w === "pyromane" ? "🔥" : "🏡";
          const label = w === "wolves" ? "Les Loups-Garous ont gagné !" : w === "angel" ? "L'Ange a gagné seul !" : w === "whitewolf" ? "Le LG Blanc a gagné seul !" : w === "flute" ? "Le Joueur de Flûte a ensorcelé tout le monde !" : w === "pyromane" ? "Le Pyromane a tout brûlé — victoire solitaire !" : "Le Village a gagné !";
          return (
        ...
        {phase === "end" && winner && (
          <>
            <div className="mx-5 mt-4 p-5 rounded-2xl text-center" style={{
              background: winner === "wolves" ? "rgba(139,28,28,0.35)" : winner === "angel" ? "rgba(168,85,247,0.2)" : winner === "whitewolf" ? "rgba(99,102,241,0.2)" : winner === "flute" ? "rgba(168,85,247,0.2)" : winner === "pyromane" ? "rgba(234,88,12,0.2)" : "rgba(16,185,129,0.15)",
              border: `1px solid ${winner === "wolves" ? "rgba(139,28,28,0.6)" : winner === "angel" ? "rgba(168,85,247,0.5)" : winner === "whitewolf" ? "rgba(99,102,241,0.5)" : winner === "flute" ? "rgba(192,132,252,0.5)" : winner === "pyromane" ? "rgba(251,146,60,0.5)" : "rgba(16,185,129,0.4)"}`,
              boxShadow: winner === "wolves" ? "0 0 30px rgba(139,28,28,0.2)" : winner === "angel" ? "0 0 30px rgba(168,85,247,0.15)" : winner === "whitewolf" ? "0 0 30px rgba(99,102,241,0.15)" : winner === "flute" ? "0 0 30px rgba(168,85,247,0.15)" : winner === "pyromane" ? "0 0 30px rgba(234,88,12,0.15)" : "0 0 30px rgba(16,185,129,0.1)",
            }}>
              <p className="text-3xl mb-2">{winner === "wolves" ? "🐺" : winner === "angel" ? "😇" : winner === "whitewolf" ? "🐺" : winner === "flute" ? "🎵" : winner === "pyromane" ? "🔥" : "🏡"}</p>
              <p className="text-lg font-bold" style={{ fontFamily: "var(--font-display)", color: winner === "wolves" ? "#f87171" : winner === "angel" ? "#c084fc" : winner === "whitewolf" ? "#818cf8" : winner === "flute" ? "#c084fc" : winner === "pyromane" ? "#fb923c" : "#34d399" }}>
                {winner === "wolves" ? "Les Loups ont gagné !" : winner === "angel" ? "L'Ange a gagné seul !" : winner === "whitewolf" ? "Le LG Blanc a gagné seul !" : winner === "flute" ? "Le Joueur de Flûte a ensorcelé tout le monde !" : winner === "pyromane" ? "Le Pyromane a tout brûlé !" : "Le Village a gagné !"}
              </p>
            </div>
